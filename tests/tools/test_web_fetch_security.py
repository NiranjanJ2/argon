...
from argon.tools.web import WebFetchTool
...
async def test_web_fetch_blocks_private_ip():
    tool = WebFetchTool()
    with patch("argon.utils.network.socket.getaddrinfo", _fake_resolve_private):
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("argon.utils.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag():
    ...
    with patch("argon.utils.network.socket.getaddrinfo", _fake_resolve_public), \
...
async def test_web_fetch_blocks_private_redirect_before_returning_image(monkeypatch):
    ...
    monkeypatch.setattr("argon.tools.web.httpx.AsyncClient", FakeClient)
