...
from argon.core.stream import StreamRenderer, ThinkingSpinner
from argon.paths import get_workspace_path, is_default_workspace
from argon.config import Config
from argon.utils.helpers import sync_workspace_templates
from argon.utils.restart import (
...
def _init_prompt_session() -> None:
    ...
    from argon.paths import get_cli_history_path
...
def onboard(
    ...
    from argon.config import get_config_path, load_config, save_config, set_config_path
    from argon.config import Config
    ...
    if wizard:
        from argon.cli.onboard import run_onboard
...
def _make_provider(config: Config):
    ...
    """
    from argon.providers.base import GenerationSettings
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from argon.config import load_config, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    import json
    from argon.config import get_config_path
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from argon.paths import get_cron_dir
...
def serve(
    ...
    from argon.api.server import create_app
...
def gateway(
    ...
    from argon.services.cron import CronService
    from argon.services.cron import CronJob
    ...
    from argon.api.server import start_dashboard, register_chat_handler, register_pushcut_handler
...
def google_auth(
    ...
    from argon.config import get_config_path, load_config, save_config, set_config_path
    from argon.google.auth import GoogleAuth, ACCOUNT_SCOPES
    ...
    if config:
        from argon.config import set_config_path
...
def agent(
    ...
    if message:
        ...
        from argon.core.bus import InboundMessage
...
def _get_bridge_dir() -> Path:
    ...
    from argon.paths import get_bridge_install_dir
...
def plugins_list():
    ...
    from argon.channels.registry import discover_all, discover_channel_names
...
def status():
    """Show nanobot status."""
    from argon.config import get_config_path, load_config
    ...
    if config_path.exists():
        from argon.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from argon.providers.github_copilot_provider import login_github_copilot
